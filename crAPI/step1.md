...
`git clone https://github.com/OWASP/crAPI.git`{{exec}}

`cd crAPI`{{exec}}

`docker-compose version`{{exec}} => 1.27.0

#### stable verion

`curl -o docker-compose.yml https://raw.githubusercontent.com/OWASP/crAPI/main/deploy/docker/docker-compose.yml`{{exec}}

`docker-compose pull`{{exec}}

`docker-compose -f docker-compose.yml --compatibility up -d`{{exec}}
