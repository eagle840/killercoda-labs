...
ZAP is a Java application and can be resource-intensive during startup. Run this command to wait until the ZAP API version endpoint returns a valid response:

`until curl -s http://127.0.0.1:8080/JSON/core/view/version | grep -q "version"; do printf '.'; sleep 5; done && echo "ZAP API is READY!"`{{exec}}

Once you see "ZAP API is READY!", you can proceed with the automation.

Check the ZAP version to confirm:

`curl -s http://127.0.0.1:8080/JSON/core/view/version | jq`{{exec}}
...
`curl -s "http://127.0.0.1:8080/JSON/ascan/view/status/" | jq`{{exec}}

## 4. Retrieve Results
Once the scan reaches 100%, you can pull the alerts in JSON format:

`curl -s "http://127.0.0.1:8080/JSON/core/view/alerts/?baseurl=http://127.0.0.1:3000" | jq`{{exec}}

Or generate a full HTML report:

`curl -s "http://127.0.0.1:8080/OTHER/core/other/htmlreport/" -o zap_api_report.html`{{exec}}
