...
`./scripts/zingg.sh --phase label --conf examples/febrl/config.json`{{exec}}  WIP:  <= can run multiple times?

answers the 20 questions on where the pairs match

`./scripts/zingg.sh --phase trainMatch --conf examples/febrl/config.json`{{exec}}

- output @ /tmp/zinggOutput/

  `ls /tmp/zinggOutput/`{{exec}}

open in editor

`cat part-0000* > output.csv`{{copy}}

- note the 3 extra columns: z_minScore, z_maxScore, z_cluster
  - sort by cluster to show ?simular items in a cluster?
  - pipe the csv into streamlit?
